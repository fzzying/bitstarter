...
if(require.main == module) {
  program
    ...
    .option('-u, --url <url_file>', 'Path to url of a file', clone(URL_Return))
    .parse(process.argv);

  // If file name was given, compare checks to file
  if(!program.url) {
    var cheon = checkHtmlFile(program.file, program.checks);
    var outJson = JSON.stringify(cheon, null, 4);
    console.log(outJson);
  }
  ...
  // If URL was given, compare checks to URL
    ...
    rest.get(program.url).on('complete', function(result) {
      ...
      var cheon = checkURL(result, program.checks);
      var outJson = JSON.stringify(cheon, null, 4);
      console.log(outJson);
    });
  }
